...
class PetriNet:
    ...
    # place/transition/arc .properties[LAYOUT_INFORMATION_PETRI]
    ...
    def extract_layout_info_to_petri_net_properties(self):
        ...
        for place in self.net.places:
            ...
            place.properties[constants.LAYOUT_INFORMATION_PETRI] = [
                ...
                [float(obj["height"]) * HEIGHT_EXTENSION_CONSTANT,
                 float(obj["width"]) * WIDTH_EXTENSION_CONSTANT]
            ]
        ...
        for item in json_dict[constants.DICT_KEY_EDGES_CONSTANT]:
            ...
            # store waiting time in the properties dictionary if target
            # is a transition (this is used to induce delay later in cpn)
            # else store the execution time in arc properties (this is used to
            # set exec time later in cpn)
            is_target_trans = isinstance(
                arc.target, pm4py.objects.petri.petrinet.PetriNet.Transition)
            if (is_target_trans):
                arc.target.properties[constants.PERFORMANCE_INFORMATION_PETRI]\
